...
description: 'Link workspace packages in monorepos (npm, yarn, pnpm, bun). USE WHEN: (1) you just created or generated new packages and need to wire up their dependencies, (2) user imports from a sibling package and needs to add it as a dependency, (3) you get resolution errors for workspace packages (@fmmenchi/*) like "cannot find module", "failed to resolve import", "TS2307", or "cannot resolve". DO NOT patch around with tsconfig paths or manual package.json edits - use the package manager''s workspace commands to fix actual linking.'
---
...
pnpm add @fmmenchi/ui --workspace

# Or with --filter from anywhere
pnpm add @fmmenchi/ui --filter @fmmenchi/app --workspace
```

Result in `package.json`:

```json
{ "dependencies": { "@fmmenchi/ui": "workspace:*" } }
```
...
```bash
yarn workspace @fmmenchi/app add @fmmenchi/ui
```
...
```bash
npm install @fmmenchi/ui --workspace @fmmenchi/app
```

Result in `package.json`:

```json
{ "dependencies": { "@fmmenchi/ui": "*" } }
```
...
```bash
cd packages/app && bun add @fmmenchi/ui
```
...
```bash
npm install @fmmenchi/data-access @fmmenchi/ui --workspace @fmmenchi/dashboard
```
...
- Symlinks appear in `<consumer>/node_modules/@fmmenchi/<package>`
