...
- **`Skeleton`.** Small and unblocked; simply behind the items above, which either unblock a
  contract or fill an empty group. Four names have now left this bullet by being built, each ahead
  of its turn and each for its own reason: `Card`, because the page shell made a grid of cards the
  obvious next thing to put in a layout and the one piece of it that is not trivial — a link that is
  visually the whole card and semantically only its title — is worth owning once rather than six
  times; `Separator`, the general-purpose `<hr>` its family separators (`MenuSeparator`,
  `ToolbarSeparator`) were already pointing at; `Breadcrumb` and `Avatar` with the table work, which
  wanted both. `Slider` left it from its own line — `<input type="range">` was deferred while its
  track and thumb were barely stylable across engines, and that is what changed.
