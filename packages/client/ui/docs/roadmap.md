...
| **Inputs**        | `Input` · `DateInput` · `TimeInput` · `DatePicker` · `DateRangePicker` · `Calendar` · `Textarea` · `Select` · `Checkbox` · `Radio` · `Switch` · `Slider` · `ChoiceField` · `InputGroup` · `Field` (+ Label, Description, Error) · `Fieldset` (+ Legend, Content) |
| **Form adapters** | `FormInput` · `FormDateInput` · `FormTimeInput` · `FormDatePicker` · `FormDateRangePicker` · `FormTextarea` · `FormSelect` · `FormChoice` · `FormSwitch` · `FormSegmentedControl` · `FormErrorSummary`                                                           |
