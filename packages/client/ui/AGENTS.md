...
- Tests split by kind: **component** behaviour in `<name>.test.tsx` (semantics, interaction, a11y via axe across variants, snapshot; RTL semantic queries + `user-event`) — **pure logic** in its own `*.test.ts` where it lives (e.g. `i18n/provider.test.tsx`), tested generically, not through a component. Vitest browser mode (Chromium).
- Storybook (`.storybook/`) with the MCP addon (`/mcp`) and a11y/docs addons; a story per component, theme + locale toolbars.
- **Component docs: a colocated `<name>.mdx`** in the standard format — fixed order, omit N/A: `<Meta of>` → `# Name` + intent → `## Anatomy` (composite only) → `## Props` (`<Controls of>`) → `## Usage` (`### State` + when-to-use + `<Canvas of>`) → `## Accessibility` → `## Guidelines` (opt) → `## i18n` (if internal copy). Every shown state is a real story (`<Canvas of>`, never ad-hoc JSX). Spec + skeleton: Storybook → **Guidelines/Component docs** (`src/docs/component-docs.mdx`); `button.mdx` is the reference.
