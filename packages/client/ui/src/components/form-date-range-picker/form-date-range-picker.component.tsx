...
function FormDateRangePicker(props: FormDateRangePickerProps) {
  ...
  // BOTH ENDS' MESSAGES, in field order, under one group. They describe the
  // group because that is what a `Fieldset` can be described by — so a library
  // whose messages do not name their field ("Required.") will read twice
  // identically, which is the library's to fix and not this component's to
  // paper over.
  const messages = [...toMessages(start.errors), ...toMessages(end.errors)];
    ...
    <Fieldset invalid={messages.length > 0}>
      ...
      <FieldsetContent>
        <DateRangePicker
          ...
          startFieldProps={startRoute.binding}
          endFieldProps={endRoute.binding}
