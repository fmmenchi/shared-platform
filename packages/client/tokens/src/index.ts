/**
 * The token CONTRACT, typed — the single enumeration of every semantic role.
 *
 * Values live in `styles/vars.css` (reference light) and `styles/presets/*`
 * (overrides); consumers never import values from TS — components read
 * `var(--fm-*)`. These types define what "a theme" IS: a valid theme assigns
 * every color role (`ThemeColors`); non-color tokens inherit unless
 * overridden. `src/tokens.test.ts` validates the CSS against this contract
 * (completeness, bridge coverage, WCAG contrast of the declared pairs).
 */
