 ...
 * through `value` and a checkbox through `checked` — two different props — so
 * it cannot produce the right one from the name alone. react-hook-form never
 * needs this because it is uncontrolled: it binds by `name` and `ref` and lets
 * the DOM hold the state either way.
 *
