...
analytics, MCP), released independently per package to GitHub Packages. **No product apps, no
services**: only abstract, independent layers consumed by external repos as `@fmmenchi/<name>`
— concretions (brand values, config, wiring) live in the consumers. The one non-published
exception is the internal docs site under `apps/` (`scope:app`), excluded from `nx release`.
